      ...
      paths: {
         ...
         [watchPath]: {
            post: {
               ...
               description:
                  `Submit the expected sender, receiver, atomic amount, and optional exact note for one top-level direct USDC asset transfer. Inner transactions, clawback transfers, and asset close-out transfers do not count as matches. The service contract includes a ${workUnitBudget}-turn work budget. ${eligibilitySummary} For unpaid discovery, RoundWatch returns HTTP 402 with PAYMENT-REQUIRED and Bazaar request metadata before semantic body validation so body-dependent clients can learn the required request shape. On the paid retry, RoundWatch parses and validates the watch specification before any facilitator verification or settlement; malformed signed input returns HTTP 400 and cannot spend. After a valid PAYMENT-SIGNATURE is settled and durable activation is confirmed, the same request returns a watch ID. The watched asset is selected by the server and is not supplied by the caller.`,
               'x-x402': x402Contract,
               requestBody: {
                  required: true,
                  content: {
               ...
               responses: {
                  ...
                  '400': {
                     description:
                        'Invalid signed watch request rejected before facilitator verification/settlement, or invalid PAYMENT-SIGNATURE header.',
                     content: {
                        'application/json': {
